...
await insertAll(db, 'recipe', [
  ...
  fakeRecipe({ householdId: household.id }),
  fakeRecipe({ householdId: household.id }),
  fakeRecipe({ householdId: secondHousehold.id }),
  fakeRecipe({ householdId: secondHousehold.id }),
])

describe('Recipe Get By HouseholdId Controller', () => {
  ...
  it('should get a collection of recipes by householdId correctly', async () => {
    await expect(
      getByHouseholdId()
