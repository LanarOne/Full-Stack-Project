  ...
  try {
    ...
    if (token) {
      ...
      if (!isHHinToken) {
        ...
        const [member, recipeResponse] = await Promise.all([
          trpc.member.getByHouseholdId.query(undefined, undefined),
          trpc.recipe.getByHouseholdId.query(undefined, undefined),
        ])
        recipes.value = recipeResponse
        const membersId = [...new Set(member.map((memberId) => memberId.userId))]
        members.value = await Promise.all(
          membersId.map((memberId) => trpc.user.getByGivenId.query({ id: Number(memberId) })),
        )
      }
