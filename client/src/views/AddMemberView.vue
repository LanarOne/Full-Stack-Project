...
async function handleSubmit() {
  ...
  try {
    ...
    await trpc.member.create.mutate({userId:user.id})
...
<template>
  <PageForm heading="Invite new member" form-label="New member" @submit="handleSubmit">
    <fwb-input v-model="userEmail" placeholder="Enter email" label="Member Email" type="email" name="email" required/>
    <fwb-button type="submit" gradient="red-yellow" :disabled="isLoading" data-testid="submitBtn">Submit</fwb-button>
  </PageForm>
</template>
